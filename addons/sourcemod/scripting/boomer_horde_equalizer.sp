...
void CheckBytes(const Address ptrAddress, int iCheckBytes[MAX_PATCH_SIZE], const int iPatchSize)
{
	...
	for (int i = 0; i < iPatchSize; i++) {
		...
		if (iCheckBytes[i] < 0 || iReadByte != iCheckBytes[i]) {
			PrintToServer("Check bytes failed. Invalid byte (read: %x@%i, expected byte: %x@%i). Check offset 'WanderersCondition'.", iReadByte, i, iCheckBytes[i], i);
			SetFailState("Check bytes failed. Invalid byte (read: %x@%i, expected byte: %x@%i). Check offset 'WanderersCondition'.", iReadByte, i, iCheckBytes[i], i);
		}
	}
}
