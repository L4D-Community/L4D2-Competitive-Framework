...
public Action EF_t_UnlockWarnMsg(Handle timer, int client)
{
...
public Action EF_t_SetDrownDmg(Handle timer, DataPack datapack)
{
...
public Action EF_t_CheckRestoring(Handle timer, int client)
{
...
public Action EF_t_StartRestoreTempHealth(Handle timer, int client)
{
...
public Action EF_t_RestoreTempHealth(Handle timer, int client)
{
	if (g_iHealthToRestore[client] <= 0 || !IsSurvivor(client)){
...
public Action EF_t_FixTempHpGlitch(Handle timer, int client)
{
...
public Action EF_t_LoadDebug(Handle timer, int client)
{
	g_hDebugTimer[client] = CreateTimer(0.1, EF_t_DebugMe, client, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}

public Action EF_t_DebugMe(Handle timer, int client)
{
